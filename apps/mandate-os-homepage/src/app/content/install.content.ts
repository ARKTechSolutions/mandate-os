...
export MANDATE_OS_MCP_DEFAULT_SOURCE="cursor.mandateos.project"`,
  ...
  },
  {
    ...
    body: 'Ask Cursor to run the approval and blocked commands below. The approval command is a dry-run package publish (`pnpm publish --dry-run`) that should pause for your consent. The blocked command is a forced delete (`rm -rf .mandateos-blocked` on macOS/Linux, or the PowerShell equivalent) that MandateOS should refuse even if you try to allow it.',
    help: 'These commands are wired into the demo mandate so you can feel allow/ask/deny without creating your own policy first. Ask Cursor to run each command; MandateOS evaluates it before the action continues.',
    codeTabs: [],
  },
];
