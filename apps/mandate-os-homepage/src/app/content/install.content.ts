...
export MANDATE_OS_MCP_DEFAULT_SOURCE="codex.mandateos.project"`,
  ...
  },
  {
    ...
    body: 'Ask Codex to run the approval and blocked commands below. The approval command is a dry-run package publish (`pnpm publish --dry-run`) that should pause for your consent. The blocked command is a forced delete (`rm -rf .mandateos-blocked` on macOS/Linux, or the PowerShell equivalent) that MandateOS should refuse even if you try to allow it. Keep MANDATE_OS_BASE_URL and MANDATE_OS_AGENT_TOKEN in the shell that launches Codex.',
...
export MANDATE_OS_MCP_DEFAULT_SOURCE="cursor.mandateos.project"`,
  ...
  },
  {
    ...
    body: 'Ask Cursor to run the approval and blocked commands below. The approval command is a dry-run package publish (`pnpm publish --dry-run`). MandateOS treats it as approval-required, but Cursor cannot complete that approval inline, so the hook stops the command and the agent should explain that MandateOS approval is required (not a permanent hard block). You cannot approve this demo from the dashboard yet — that needs a real MandateOS workspace. The blocked command is a forced delete (`rm -rf .mandateos-blocked` on macOS/Linux, or the PowerShell equivalent) that MandateOS hard-denies with no approval path.',
    help: 'These commands are wired into the demo mandate. On Cursor, approval-required actions are denied with a distinct agent message so you can still see the difference from a hard block. Full approve-and-retry needs the MandateOS control panel after you sign up.',
    codeTabs: [],
  },
];
...
export MANDATE_OS_MCP_DEFAULT_SOURCE="claude.mandateos.local"`,
  ...
  },
  {
    ...
    body: 'Ask Claude Code to run the approval and blocked commands below. The approval command is a dry-run package publish (`pnpm publish --dry-run`) that should pause for your consent. The blocked command is a forced delete (`rm -rf .mandateos-blocked` on macOS/Linux, or the PowerShell equivalent) that MandateOS should refuse even if you try to allow it. Open Claude Code in this repository first — the local-scoped mandateos MCP entry and PreToolUse hooks load automatically.',
...
npx --yes --package @mandate-os/openclaw@latest mandate-os-openclaw-install doctor`,
  ...
  },
  {
    ...
    body: 'Ask the guarded OpenClaw agent to run the approval and blocked commands below. The approval command is a dry-run package publish (`pnpm publish --dry-run`) that should pause for your consent. The blocked command is a forced delete (`rm -rf .mandateos-blocked` on macOS/Linux, or the PowerShell equivalent) that MandateOS should refuse even if you try to allow it. Start OpenClaw with the mandateos_guarded agent profile first.',
    help: 'The guarded agent profile routes shell execution and browser actions through the MandateOS bridge. These demo commands are wired into the install mandate so you can feel allow/ask/deny without creating your own policy first.',
    codeTabs: [],
  },
];
